...
from scripts.txt2img import load_model
...
style_root = "data/musicTI_dataset/audios/timbre"
logs_root = "logs"
...
def compute_clap_audio_embedding_from_mel(inputs):
...
def compute_clap_text_embedding(text):
...
for content in tqdm(content_dirs, desc="Contents"):
    ...
    content_embed = compute_clap_audio_embedding_from_mel(content_audio_path)

    # === 1. Dual Style Evaluation ===
    ...
    for style1, style2 in tqdm(style_pairs, desc=f"{content} Style Pairs", leave=False):
        ...
        interpolated_embed = compute_clap_audio_embedding_from_mel(interpolated_audio_path)
        ...
        style_1_embed = compute_clap_audio_embedding_from_mel(style_1_audio_path)
        style_2_embed = compute_clap_audio_embedding_from_mel(style_2_audio_path)
        
        style_1_text_embed = compute_clap_text_embedding(f"A music piece in the style of {style1}")
        style_2_text_embed = compute_clap_text_embedding(f"A music piece in the style of {style2}")
    ...
    for style in tqdm(style_dirs, desc=f"{content} Single Styles", leave=False):
        ...
        style_embed = compute_clap_audio_embedding_from_mel(style_audio_path)
        
        style_text_embed = compute_clap_text_embedding(f"A music piece in the style of {style}")
        content_preservation = cosine_similarity(content_embed, interpolated_embed)
...
# print(f"Style Fit Singles     {style_fit_singles}")
